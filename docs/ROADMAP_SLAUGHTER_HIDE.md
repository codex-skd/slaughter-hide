...
## Fase 3.4b1.5 — Triaje batch B2 y limitación real del sistema de cortes (2026-08-25)

Al triar el batch B2 aparecieron dos hallazgos que cambian el alcance:

1. **`CarcassCutupHandler` está hardcodeado a exactamente 3 cortes** (`cut_1`/`cut_2`/`cut_3`, más cabeza y piel opcionales) — el campo `numCuts` de `CarcassDefinition` y su `stageMap()` genérico (que sí soportan N cortes en el modelo de datos) **nunca se conectan al handler real**. Esto significa que `ENDERMAN` (declarado con `numCuts=8`) y `TURTLE` (`numCuts=7`) nunca pudieron dar más de 3 cortes en el juego — las loot tables `cut_4` en adelante existían pero eran inalcanzables. No es un crash (una loot table inexistente devuelve lista vacía, verificado en `CarcassLoot.spawn`), solo contenido incompleto silencioso.
   - **Decisión del usuario**: no generalizar el handler para soportar N cortes reales (cambio de núcleo, afecta a los 74 mobs en producción) — en su lugar, **recortar el dato a lo que realmente funciona** y documentarlo. `ENDERMAN`/`TURTLE` corregidos a `numCuts=3` en `Carcasses.java` (antes 8/7) para que el modelo de datos deje de prometer algo que el juego no da. Las loot tables `cut_4..cut_8`/`cut_4..cut_7` siguen en `temp/butchery-assets` (no tocadas, fuera del repo real) pero no se han portado ni se portarán salvo que se decida generalizar el handler en el futuro.
2. **`Ravager` y `Pufferfish`, del catálogo de "54 bloques únicos" (candidatos a batch B2), resultaron ser el mecanismo *bespoke* original de carcasa para esos dos mobs** — no contenido nuevo. Ambos ya tenían una `CarcassDefinition` genérica en nuestro port (parte de los 74 mobs), pero **rota**: registraban un bloque `<mob>_carcass` sin blockstate/modelo real (el original nombra estos assets `ravager.json`/`pufferfish.json`, sin sufijo `_carcass`).
   - **Pufferfish**: sus 2 cortes ya usaban la convención `cut_N` correcta y esas loot tables sí existen — solo faltaba el asset de la carcasa fresca. Arreglado copiando/remapeando `pufferfish.json` → `pufferfish_carcass.json` (+ modelos + textura), sin tocar `Carcasses.java` (su `numCuts=2` ya era correcto).
   - **Ravager**: su ciclo real en el original no es "cabeza+piel+3 cortes genéricos" — es cabeza + 4 patas + cuerpo (`ravager_head_drop`, `ravager_leg_1..4_drop`, `ravager_body_drop`), con seguimiento de estado por NBT en un `RavagerBlockEntity` propio, sin cambiar de blockstate visualmente entre cortes. No encaja en el patrón genérico. Se aplicó solo el arreglo cosmético mínimo (asset `ravager_carcass.json` copiado desde `ravager.json`, aunque el modelo original no referencia una textura de bloque estática — probablemente se renderizaba vía un `BlockEntityRenderer` propio en `client/`, Fase 4, no portada) para que al menos no aparezca como "missing model". El ciclo de despiece real de Ravager (5 etapas con esos nombres) **no está portado** — descartado deliberadamente para no tocar el núcleo genérico, documentado aquí en vez de intentarlo.
...
**RESUELTO (2026-08-25)**: investigado y confirmado seguro — 154 archivos en la carpeta plural, de los cuales 136 eran exclusivos (no existían en `loot_table/` singular) y 18 eran duplicados byte-idénticos ya presentes en la ruta correcta (sin conflictos de contenido). Movidos los 136 con `git mv`, eliminados los 18 duplicados tras confirmar identidad exacta. Cobertura real del bug: prácticamente todos los mobs humanoides (`drowned`, `evoker`, `husk`, `piglin`, `piglin_brute`, `skeleton`, `vindicator`, `witch`, `zombie`) y los 4 "especiales" (`enderman`, `endermite`, `sniffer`, `strider`, `turtle`) + drops de `ravager` — ver `CHANGELOG.md` [Unreleased].
